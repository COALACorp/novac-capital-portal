...
type UploadFileButtonProps = {
    fileName: string,
    onChange?: (fileName: string, value: File) => void,
    onUpload?: () => void,
};
...
function UploadFileButton(props: UploadFileButtonProps) {
    ...
    useEffect(() => {
        if (file && props.onChange)
            props.onChange(props.fileName, file);
    }, [file]);

    return file ? (
        <>
            <RemoveFileButton file={file} onRemove={handleRemove} />
            <Box className="file-input-action send-file" component="a" onClick={props.onUpload}>
                <p>Enviar</p>
            </Box>
        </>
    ) : (
        <label className="file-input-action" htmlFor={"file-" + props.fileName}>
            <p>Subir</p>
            ...
            <input id={"file-" + props.fileName} type="file" onChange={handleChange} />
        </label>
    );
}
